...
function joint = parse_sw_mp(filename)
% =========================================================================
% Parses Solidworks mass properties dialog when the output of the "Copy to
% Clipboard" button is pasted into a text file. Assumes you've set the
% decimal precision in SW to the max (8)
...
mp = mp(6:end); % remove first instance of the word 'Mass'
    ...
    function val = findVal(varname)
        ...
        chars = 9;
        if (strcmp(mp(index:index),'-') == 1)
            chars = 10;
        end
        val = mp(index:index+chars);
    end
